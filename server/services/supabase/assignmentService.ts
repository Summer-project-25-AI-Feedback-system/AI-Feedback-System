...
export const createOrUpdateAssignments = async (organizationId: string, assignments: AssignmentInput | AssignmentInput[]) => {
  ...
  const incomingAssignmentIds = new Set(assignmentsArray.map((a) => a.external_github_assignment_id));
  const existingAssignmentIds = new Set((existingAssignments || []).map((o) => o.external_github_assignment_id));
  ...
  if (assignmentsToDelete.length > 0) {
    ...
    if (deleteError) {
      throw new Error('Failed to delete assignments from DB that no longer exist in GitHub.');
    }
  }

...
import { fetchAssignments, createOrUpdateAssignments } from '../../services/supabase/assignmentService';
...
export const getAssignments = async (req: Request, res: Response) => {
  ...
  try {
    const assignments = await fetchAssignments(organizationId, github_assignment_id as string | undefined);
    ...
    res.status(500).json({ error: 'Unexpected server error while retrieving assignment(s)' });
  }
};
